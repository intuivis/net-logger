        ...
        useEffect(() => {
            ...
            const fetchInitialData = async () => {
                try {
                    ...
                    const { data: netDataRow, error: netError } = await supabase
                        .from('nets')
                        .select('*')
            ...
            channel
                .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'check_ins', filter: `session_id=eq.${sessionId}`}, (payload) => {
                    const newCheckInFromDb = payload.new as Database['public']['Tables']['check_ins']['Row'];
                    ...
                    setCheckIns(current => {
                        ...
                        if (optimisticEntry) {
                            return current.map(c => c.id === optimisticEntry.id ? { ...newCheckInFromDb, isOptimistic: false, status_flag: newCheckInFromDb.status_flag as CheckInStatusValue } : c);
                        }
                ...
                .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'check_ins', filter: `session_id=eq.${sessionId}`}, (payload) => {
                    const updatedCheckIn = payload.new as any as CheckIn;
